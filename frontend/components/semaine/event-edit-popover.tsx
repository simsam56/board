...
interface EventEditPopoverProps {
  event: PlannerEvent;
  onSave: (event: PlannerEvent, startAt: string, endAt: string, title: string) => void;
  onClose: () => void;
  isPending?: boolean;
}

export function EventEditPopover({ event, onSave, onClose, isPending }: EventEditPopoverProps) {
  ...
  const handleSave = () => {
    ...
    onSave(event, startAt, endAt, title.trim() || event.title);
  };
      ...
      <div className="glass w-full max-w-sm rounded-2xl p-5">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs text-text-muted">Debut</label>
              <input
                type="time"
                value={startTime}
